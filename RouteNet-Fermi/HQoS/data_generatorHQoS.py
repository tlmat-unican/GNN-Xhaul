...
POLICIES = np.array(['WFQ', 'SP', 'DRR', 'FIFO', "SPWFQ"])


def generator(data_dir, shuffle, seed, format="decimal"):
    try:
        data_dir = data_dir.decode('UTF-8')
    except (UnicodeDecodeError, AttributeError):
        pass
    tool = DatanetAPI(data_dir, shuffle=shuffle, seed=seed)
    # print("DatanetAPI initialized with data directory: {}".format(data_dir))
    it = iter(tool)
    num_samples = 0
    for sample in it:
        
        G = nx.DiGraph(sample.get_topology_object())
        # print("Graph matrix: {}".format(G))
        T = sample.get_traffic_matrix()
        # print("Traffic matrix: {}".format(T))
        R = sample.get_routing_matrix()
        # print("Routing matrix: {}".format(R))   
        P = sample.get_performance_matrix()
        # print("Performance matrix: {}".format(P))
        # if format == "decimal":
        HG = network_to_hypergraph_decimal(G=G, R=R, T=T, P=P)
        # else:
        #     HG = network_to_hypergraph_ln(G=G, R=R, T=T, P=P)
        # print("HG details")
        # print("Number of nodes: {}".format(HG.number_of_nodes()))
        # print("Number of edges: {}".format(HG.number_of_edges()))
        # print("Node types: {}".format(set([n.split('_')[0] for n in HG.nodes()])))
...
def hypergraph_to_input_data(HG):
    ...
    # print("packets: {}".format(list(nx.get_node_attributes(HG, 'packets').values())))
...
def network_to_hypergraph_decimal(G, R, T, P):
    # print("routing matrix: {}".format(R))

 
    # print("routing matrix after processing: {}".format(R))
    D_G = nx.DiGraph()
    for src in range(G.number_of_nodes()):
        for dst in range(G.number_of_nodes()):
            if src != dst:
                # print("Processing src-dst node {}/{}".format(src, dst))
                if G.has_edge(src, dst):
                    D_G.add_node('l_{}_{}'.format(src, dst),
                                 capacity=G.edges[src, dst]['bandwidth'],
                                 policy=np.where(G.nodes[src]['schedulingPolicy'] == POLICIES)[0][0])
                    # print("  - added node since there is and edge in the graph from {} to {}".format(src, dst))
                # print("Processing src-dst pair {}/{}".format(src, dst), " with {} flows".format(len(T[src, dst]['Flows'])))
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        # print("Processing flow from {} to {} with id {}".format(src, dst, f_id))
                        ...
                        if 'EqLambda' in flow['TimeDistParams']:
                            time_dist_params[0] = flow['TimeDistParams']['EqLambda'] #Bytes per second
                            # print(f"EqLambda for flow {src}->{dst} with id {f_id}: {time_dist_params[0]}")
                        if 'AvgPktsLambda' in flow['TimeDistParams']:
                            time_dist_params[1] = flow['TimeDistParams']['AvgPktsLambda'] # avg. Packets per second
                            # print(f"AvgPktsLambda for flow {src}->{dst} with id {f_id}: {time_dist_params[1]}")
                        if 'ExpMaxFactor' in flow['TimeDistParams']:
                            time_dist_params[2] = flow['TimeDistParams']['ExpMaxFactor'] # bound for the exponential distribution of inter-arrival times
                        if 'PktsLambdaOn' in flow['TimeDistParams']:
                            time_dist_params[3] = flow['TimeDistParams']['PktsLambdaOn'] # Gross packet generation rate when the source is active (in pkts/s)
                        if 'AvgTOff' in flow['TimeDistParams']:
                            time_dist_params[4] = flow['TimeDistParams']['AvgTOff'] #  duration of the OFF periods (in seconds) CTE
                        if 'AvgTOn' in flow['TimeDistParams']:
                            time_dist_params[5] = flow['TimeDistParams']['AvgTOn']  #  duration of the ON periods (in seconds) CTE 
                    ...
                    for h_1, h_2 in [R[src, dst][i:i + 2] for i in range(0, len(R[src, dst]) - 1)]:
                        ...
                        if 'bufferSizes' in G.nodes[h_1]:
                            ...
                            q_s = [int(q)*1000 for q in str(G.nodes[h_1]['queueSizes']).split(',')]
                        # policy = G.nodes[h_1]['schedulingPolicy']
                        # print("queue sizes")
                        policy = G.nodes[h_1]['schedulingPolicy']
                        if G.nodes[h_1]['schedulingWeights'] != '-':
                            raw_weights = [float(w) for w in str(G.nodes[h_1]['schedulingWeights']).split(',')]
                            num_queues = G.nodes[h_1]['levelsQoS']
                            missing_weights = num_queues - len(raw_weights)
                            if missing_weights > 0:
                                q_w = [0.0] * missing_weights + raw_weights
                            else:
                                q_w = raw_weights[:num_queues]
                            w_sum = sum(q_w)
                            if w_sum > 0 and G.nodes[h_1]['schedulingPolicy'] in ['WRR', 'WFQ', 'SPWRR', 'SPWFQ']:
                                q_w = [w / w_sum for w in q_w]
                            else:
                                q_w = [0.0] * num_queues
                        else:
                            q_w = [0.0] * G.nodes[h_1]['levelsQoS']
...
def network_to_hypergraph_ln(G, R, T, P):
    # print("routing matrix: {}".format(R))
                ...
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        ...
                        D_G.add_node('p_{}_{}_{}'.format(src, dst, f_id),
                                     ...
                                     ar_a=time_dist_params[6],
                                     sigma=time_dist_params[7],
                                     delay=P[src, dst]['Flows'][f_id]['AvgLnDelay'])
...
def input_fn(data_dir, shuffle=False, seed=1234, format="decimal"):
    ds = tf.data.Dataset.from_generator(generator,
                                        args=[data_dir, shuffle, seed, format],
                                        output_signature=(
