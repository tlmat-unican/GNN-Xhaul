...
class RouteNet_Fermi(tf.keras.Model):
    def __init__(self):
        ...
        self.path_embedding = tf.keras.Sequential([
            tf.keras.layers.Input(shape=10 + self.max_num_models), # 10 son metricas fisicas, maxnum_modes vector one hot 
            tf.keras.layers.Dense(self.path_state_dim, activation=tf.keras.activations.relu), # primera capa oculata del mlp tranformacion linal y no lineal RELU
            tf.keras.layers.Dense(self.path_state_dim, activation=tf.keras.activations.relu) ## para refinar los datos
        ])
        ...
        self.readout_path = tf.keras.Sequential([
            tf.keras.layers.Input(shape=(None, self.path_state_dim)), # entry 32 dim
            tf.keras.layers.Dense(int(self.link_state_dim / 2), # Capa 16 neurons
                                  activation=tf.keras.activations.relu),
            tf.keras.layers.Dense(int(self.path_state_dim / 2), # Capa 16 neurons
                                  activation=tf.keras.activations.relu),
            tf.keras.layers.Dense(1)  # capa 1 neuron
        ], name="PathReadout")
    ...
    def call(self, inputs):
        ...
        # traffic en bits/s dividido por paquetes/s = bits/paquete   - params from flowSimresultsdataset
        # Sustituye tu print actual por este:
        # tf.print("Valores de pkt_size:", pkt_size/8, summarize=-1)
        ...
        # tf.print ("traffic normalizado:", (traffic - self.z_score['traffic'][0]) / self.z_score['traffic'][1], summarize=-1)
        # tf.print ("packets normalizado:", (packets - self.z_score['packets'][0]) / self.z_score['packets'][1], summarize=-1)
        # tf.print ("eq_lambda normalizado:", (eq_lambda - self.z_score['eq_lambda'][0]) / self.z_score['eq_lambda'][1], summarize=-1)
        # tf.print ("avg_pkts_lambda normalizado:", (avg_pkts_lambda - self.z_score['avg_pkts_lambda'][0]) / self.z_score['avg_pkts_lambda'][1], summarize=-1)
        # tf.print ("exp_max_factor normalizado:", (exp_max_factor - self.z_score['exp_max_factor'][0]) / self.z_score['exp_max_factor'][1], summarize=-1)
        # tf.print ("pkts_lambda_on normalizado:", (pkts_lambda_on - self.z_score['pkts_lambda_on'][0]) / self.z_score['pkts_lambda_on'][1], summarize=-1)
        # tf.print ("avg_t_off normalizado:", (avg_t_off - self.z_score['avg_t_off'][0]) / self.z_score['avg_t_off'][1], summarize=-1)
        # tf.print ("avg_t_on normalizado:", (avg_t_on - self.z_score['avg_t_on'][0]) / self.z_score['avg_t_on'][1], summarize=-1)
        # tf.print ("ar_a normalizado:", (ar_a - self.z_score['ar_a'][0]) / self.z_score['ar_a'][1], summarize=-1)
        # tf.print ("sigma normalizado:", (sigma - self.z_score['sigma'][0]) / self.z_score['sigma'][1], summarize=-1)  
