...
class PointToPointNetDevice : public NetDevice
{
  ...
  private:
    ...
    /**
     * @brief The receive queue for the point-to-point net device.
     * 
     * This queue stores incoming packets that are received by the point-to-point net device.
     * It allows for buffering of packets until they can be processed by the device.
     * The type of the queue is `Ptr<Queue<Packet>>`.
     */

    Ptr<DropTailQueue<Packet>> m_queuerx;
    ...
    DataRate m_tSwitchingCapacity;
    bool m_enableSwitchingTime;
    bool m_model_enable;
    /**
     *Enumeration of the states of the RECIEVED machine of the net device.
    */
     enum RxMachineState
    {
        ON, /**< The transmitter is ready to begin transmission of a packet */
        OFF   /**< The transmitter is busy transmitting a packet */
    };
   
    RxMachineState m_rxMachineState;

    void SwitchingStart(Ptr<Packet> p);
    Time CalculateSwitchingTime(uint32_t bytes);
    void SwitchingComplete(Ptr<Packet> p);
    ...
    TracedCallback<Ptr<const Packet>, const std::string&> m_promiscSinfferActionTrace, m_promiscSnifferActionrxTrace, m_snifferTwait, m_snifferTpw; 

    TracedCallback<const std::string&> m_snifferTps, m_snifferTs;

    std::string m_switchingDistribution;
    Ptr<RandomVariableStream> m_time_id;
    Time m_tSwitchingTime, m_tMinSwitchingTime, m_tMaxSwitchingTime;
    ...
    Ptr<Packet> m_currentPkt, m_currentPktrx; //!< Current packet processed
