...
FifoQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item)
{
    ...
    // std::cout <<this << " " <<"Enqueue pkt of size " << item->GetSize() << " bytes" << std::endl;
    // std::cout << Simulator::Now().GetNanoSeconds() << " " << item << " FIFO Enqueue: Current queue size: " << GetCurrentSize() << " bytes vs. Max: " << GetMaxSize() << std::endl;
    if (GetCurrentSize() >= GetMaxSize())
    {
        NS_LOG_LOGIC("Queue full -- dropping pkt");
        // std::cout <<"Drop pkt with currentsize: " << GetCurrentSize() << " and maxsize: " << GetMaxSize() << std::endl;
    ...
    // std::cout << "Number pkts " << GetInternalQueue(0)->GetNPackets() << std::endl;
    return retval;
}
