...
QueueDisc::PacketEnqueued(Ptr<const QueueDiscItem> item)
{
    ...
    m_accSojourntime += item->GetTimeStamp().GetNanoSeconds();
...
QueueDisc::DropBeforeEnqueue(Ptr<const QueueDiscItem> item, const char* reason)
{
    ...
    std::cout << "DropBeforeEnqueue: " << reason << std::endl;
    ...
    if (item->GetTimeStamp().GetNanoSeconds() > 0)
    {
        uint64_t dropEntryTime = item->GetTimeStamp().GetNanoSeconds();
        if (m_accSojourntime >= dropEntryTime)
        {
            m_accSojourntime -= dropEntryTime;
        }
    }
    
    
    if (m_nPackets == 0) m_accSojourntime = 0;
    m_traceDrop(item);
...
QueueDisc::Enqueue(Ptr<QueueDiscItem> item)
{
    ...
    // Si DoEnqueue falló internamente, ns-3 ya habrá llamado a DropBeforeEnqueue
    // por lo que nuestra lógica de limpieza en DropBeforeEnqueue se activará.
...
QueueDisc::Dequeue()
{
    ...
    {   
        // std::cout << "QueueDisc::DoDequeue() " << Simulator::Now().GetFemtoSeconds() << std::endl;
        item = DoDequeue();
    }
...
uint64_t 
QueueDisc::GetTotalAccTimestamp() const{

    return m_accSojourntime;
}
