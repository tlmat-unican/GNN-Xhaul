...
class OnOffApplication : public Application
{
  ...
  private:
    ...
    bool m_startInOnState;
    bool m_connected;                    //!< True if connected
    uint8_t m_dscp;                      //!< DSCP value to set in the IP header of generated packets
    Ptr<RandomVariableStream> m_onTime;  //!< rng for On Time
    Ptr<RandomVariableStream> m_offTime; //!< rng for Off Time
    Ptr<RandomVariableStream> packetsize; //!< rng for packet size
    DataRate m_cbrRate;                  //!< Rate that data is generated
    DataRate m_cbrRateFailSafe;          //!< Rate that data is generated (check copy)
    uint32_t m_pktSize, m_MaxpktSize;                  //!< Size of packets
    uint32_t m_pktSize_cte;              //!< Size of packets (check copy)
