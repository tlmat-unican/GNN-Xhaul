    ...
    static TypeId tid =
        TypeId("ns3::OnOffApplication")
            ...
            .AddAttribute("DataRate",
                          ...
                          MakeDataRateChecker())
            .AddAttribute("MaxPktSize",
                          "The size of packets sent in on state",
                          UintegerValue(512),
                          MakeUintegerAccessor(&OnOffApplication::m_MaxpktSize),
                          MakeUintegerChecker<uint32_t>(1))
            .AddAttribute("PacketSize",
                          "The size of packets sent in on state",
                          UintegerValue(1),
                          MakeUintegerAccessor(&OnOffApplication::m_pktSize_cte),
                          MakeUintegerChecker<uint32_t>(1))
            .AddAttribute("Packetgen",
                         "A RandomVariableStream used to pick the length of payload.",
                          StringValue("ns3::ConstantRandomVariable[Constant=1.0]"),
                          MakePointerAccessor(&OnOffApplication::packetsize),
            ...
            .AddAttribute("OffTime",
                          ...
                          MakePointerChecker<RandomVariableStream>())
            .AddAttribute("StartInOnState", 
                            "Whether the application starts in ON state",
                            BooleanValue(false), // Por defecto sigue igual (empieza en OFF)
                            MakeBooleanAccessor(&OnOffApplication::m_startInOnState),
                            MakeBooleanChecker())
            .AddAttribute("DSCP",
                          "DSCP value to set in the IP header of generated packets (0-63)",
                          UintegerValue(0),
                          MakeUintegerAccessor(&OnOffApplication::m_dscp),
...
OnOffApplication::StartApplication() // Called at time specified by Start
{
    ...
    {
        if (m_startInOnState) {
            StartSending(); // Empieza inmediatamente
        } else {
            ScheduleStartEvent(); // Comportamiento original (espera el OffTime)
        }
    }
}
...
OnOffApplication::ScheduleNextTx()
{
    ...
    if(m_pktSize_cte == 1){
        ...
        m_pktSize = m_pktSize_cte;
    }
        // std::cout << "Packet size: " << aux << std::endl;
...
OnOffApplication::SendPacket()
{
    ...
    if (m_pktSize_cte == 1){
       ...
       if (m_pktSize == 0){
        ...
        m_pktSize = m_pktSize_cte;
        // std::cout << "Packet size: " << m_pktSize << std::endl;
    }
    
    // std::cout << "Packet size: " << aux << std::endl;
    if (m_unsentPacket)
    {   
        // std::cout << "m_unsentPacket" << std::endl;
