...
int main(int argc, char *argv[]) {
    ...
    cmd.AddValue("trafficFile", "Input traffic JSON file", path);
        ...
        for (uint32_t i = 0; i < 2; ++i) { 
            ...
            if (spec.mapQueue != "0 0"){
                if (cfg.policy == "WRR") {
                    ...
                    std::string bwNumericPart = e.bw.substr(0, e.bw.find("bps"));

                    // 2. Convertimos a double y dividimos para pasar de bps a Gbps
                    double bwInGbps = std::stod(bwNumericPart) / 1e9;
                                        
                    tch.AddChildQueueDisc(roothandle, cid[1], "ns3::WfqQueueDisc", 
                                        "Quantum", StringValue(WeightsToIntegers(cfg.weights)), 
                                        "MapQueue", StringValue(DecrementMapQueue(spec.mapQueue)), 
                                        "MaxSize", StringValue("1000p"), "ChannelDataRate", DoubleValue(bwInGbps));
    ...
    if (i->second.rxPackets > 0) {
        avgHops = (double)i->second.timesForwarded / i->second.rxPackets + 1;
    }
    std::cout << "Flow " << i->first << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
    std::cout << "  Saltos (Hops): " << avgHops << "\n";
    std::cout << "  Paquetes Recibidos: " << i->second.rxPackets << "\n";
    std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds()) / 1024 / 1024  << " Mbps\n";
    std::cout << "  Delay Medio: " << (i->second.delaySum.GetSeconds() / i->second.rxPackets )*1e6<< " us\n";
    std::cout << "------------------------------------------\n";
    }

    // monitor->SerializeToXmlFile("resultados_completos.xml", true, true);
